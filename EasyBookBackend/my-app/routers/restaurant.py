...
async def update_restaurant_info(UpdateRestaurantRequest: schema.UpdateRestaurantRequest, db: Session = Depends(get_db)):
...
async def add_to_blacklist(AddToBlacklistRequest:schema.AddToBlacklistRequest,db: Session = Depends(get_db)):

    return await business.add_to_blacklist(AddToBlacklistRequest,db)


@router.post("/add_consumption_record", summary="新增會員消費紀錄")
async def add_consumption_record(AddConsumptionRecordRequest: schema.AddConsumptionRecordRequest,db: Session = Depends(get_db)):
